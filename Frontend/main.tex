...
\newcommand{\missingimage}[2]{%
  \fbox{\parbox{0.85\textwidth}{\centering \textbf{Image manquante :} #1}}%
  \caption{#2}%
}
\usepackage{tikz}
\usetikzlibrary{calc}
...
Le domaine de la peinture couvre trois grandes familles de pratiques : la peinture artistique, la peinture en bâtiment et la peinture décorative. Ces métiers partagent des savoirs fondamentaux mais se distinguent par leurs formations, leurs débouchés économiques et leurs règles professionnelles strictes.

Aujourd'hui, l'information relative au métier de peintre est fragmentée, dispersée entre des sites institutionnels, des organisations professionnelles et des forums en ligne. Il n'existe \textbf{aucune plateforme unique, structurée et maintenue par les peintres eux-mêmes} qui centralise les savoirs du métier : techniques, outils, matériaux, formations, réglementation.

\subsubsection*{Problématique}

Comment permettre à toute personne intéressée par les métiers de la peinture (étudiant, amateur, professionnel) d'accéder facilement à une information fiable, structurée et enrichie par les praticiens eux-mêmes ?

Sans cette solution, un étudiant souhaitant devenir peintre doit naviguer entre une dizaine de sites différents, sans savoir quelle information est fiable ni par où commencer. Les peintres professionnels, quant à eux, ne disposent d'aucun espace dédié pour partager leur expertise et faire connaître leur travail.
...
\subsubsection*{Fonctionnalités pour le Visiteur (sans compte)}

\begin{itemize}[leftmargin=1.5cm]
  \item Consulter la liste des fiches métier par catégorie
  \item Rechercher un peintre par nom, spécialité ou localisation géographique
  \item Visualiser le détail d'une fiche (description, compétences requises, médias, tags)
  \item Géolocaliser les peintres sur une carte interactive (Leaflet.js)
  \item Accéder à la plateforme depuis un navigateur mobile ou desktop sans friction
\end{itemize}

\subsubsection*{Fonctionnalités pour le Peintre (connecté et validé)}

\begin{itemize}[leftmargin=1.5cm]
  \item Créer et modifier son profil professionnel (nom, spécialité, biographie, localisation)
  \item Publier des fiches métier (techniques, formations, tutoriels, bonnes pratiques)
  \item Modifier ou archiver ses propres publications
  \item Participer à la modération communautaire (signaler des contenus non-conformes)
  \item Consulter ses statistiques de contribution (nombre de fiches publiées, consultations)
\end{itemize}

\subsubsection*{Fonctionnalités pour l'Administrateur}

\begin{itemize}[leftmargin=1.5cm]
  \item Valider ou rejeter les demandes d'inscription des peintres
  \item Modérer les contenus publiés (suppression si non-conformes, appel à révision)
  \item Créer, modifier ou supprimer des catégories et tags de classification
  \item Suspendre un compte peintre en cas d'abus ou de violation des règles
  \item Consulter un tableau de bord des statistiques globales de la plateforme
\end{itemize}

\subsubsection*{Règles transverses}

\begin{itemize}[leftmargin=1.5cm]
  \item Seul un peintre avec compte validé par un administrateur peut publier du contenu
  \item Les fiches doivent suivre un format structuré défini : titre, catégorie, corps, tags obligatoires
  \item Chaque requête API est tracée avec l'identifiant utilisateur (traçabilité BCaaS)
  \item L'application fonctionne en mode dégradé sans internet (offline-first, PWA)
  \item Les contenus sensibles (contacts, données personnelles) sont filtrés avant affichage
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Stack technique du projet Painting Business Core}
...
\hline
Frontend & Next.js 14 (App Router) & SEO natif, React moderne, internationalisation i18n intégrée, déploiement simplifié \\
\hline
Backend API & Spring Boot 3 (simulation en Phase 1) & Robustesse éprouvée, sécurité mature, intégration facile avec PostgreSQL \\
\hline
Base de données & PostgreSQL + PostGIS & Recherche full-text, requêtes spatiales pour la géolocalisation \\
\hline
Cache distribué & Redis 7 & Gestion des sessions, cache de requêtes fréquentes \\
\hline
Stockage médias & MinIO (S3-compatible) & Auto-hébergé, open source, compatible avec NextJS \\
\hline
Conteneurisation & Docker + Docker Compose & Reproductibilité des environnements (dev, test, prod) \\
\hline
Authentification & JWT + OAuth2 + 2FA & Sécurité stateless, support multi-tenant \\
\hline
\end{tabular}
\end{table}

\subsubsection*{Contraintes techniques mesurables}

\begin{itemize}[leftmargin=1.5cm]
  \item \textbf{Tous les composants} doivent être open source ou gratuits pour usage éducatif
  \item \textbf{Déploiement} sur les serveurs de l'université ou en mode localhost (portable)
  \item \textbf{Fichiers uploadés} : taille maximale 10 Mo, formats acceptés : JPG, PNG, WebP, MP4
  \item \textbf{Sécurité} : HTTPS obligatoire, en-têtes de sécurité (CSP, HSTS, X-Frame-Options)
  \item \textbf{Performance} : temps de chargement cible $< 2$ secondes sur connexion 4G
  \item \textbf{PWA} : score Lighthouse PWA $\geq 90$, score Performance $\geq 90$, LCP $< 2,0$s
  \item \textbf{Accessibilité} : conformité WCAG 2.1 niveau AA minimum
  \item \textbf{Internationalisation} : support de plusieurs langues (français, anglais minimum)
  \item \textbf{Traçabilité} : chaque transaction loggée avec actor\_id, trace\_id, timestamp, operation
\end{itemize}

\subsubsection*{Technologies exclues (justification)}

\begin{table}[H]
\centering
\caption{Technologies exclues du projet --- Justification}
\label{tab:excluded_tech}
\small
\begin{tabular}{|p{0.25\textwidth}|p{0.68\textwidth}|}
\hline
\textbf{Technologie exclue} & \textbf{Raison} \\
\hline
Application mobile native (iOS/Android) & PWA offre une couverture suffisante pour mobile, développement plus rapide \\
\hline
GraphQL & REST avec stratégie d'abstraction API (lib/api.ts) suffit aux besoins ; GraphQL surcharge pour un MVP \\
\hline
Apache Kafka & Surdimensionné pour un projet étudiant ; streaming d'événements non critique \\
\hline
Spring WebFlux / Reactive & Courbe d'apprentissage élevée ; Spring Boot classique suffisant pour les besoins \\
\hline
MongoDB (NoSQL) & PostgreSQL avec support JSON natif offre plus de flexibilité pour les recherches \\
\hline
\end{tabular}
\end{table}
...
% ============================================================
%  SECTION 3 — ANALYSE FONCTIONNELLE ET ARCHITECTURE BCAAS
% ============================================================
\section{Analyse fonctionnelle et architecture BCaaS}
...
Cette section constitue le socle analytique du projet \textit{Painting Business Core Instance}. Elle rassemble l'état de l'art des solutions existantes, la modélisation détaillée des besoins de chaque acteur, et justifie le positionnement architectural de notre instance au sein de l'écosystème BCaaS.
...
% ============================================================
%  SECTION 2 — CAHIER DES CHARGES (SOUS-SECTION)
% ============================================================
...
% ============================================================
\subsection{Framework principal : Next.js 14 (App Router)}
% ============================================================
...
% ============================================================
\subsection{Couche de persistance : Mock Data (Fichiers JSON structurés)}
% ============================================================
...
% ============================================================
\subsection{Moteur d'abstraction API : \texttt{lib/api.ts}}
% ============================================================
...
% ============================================================
\subsection{Tableau récapitulatif des choix technologiques}
% ============================================================
...
% ============================================================
%  SECTION 4 — CONCEPTION ARCHITECTURALE ET MODÉLISATION
% ============================================================
...
\begin{figure}[H]
    \centering
    \missingimage{useCase.jpeg}{Diagramme des cas d'utilisation}
    \label{fig:use_case}
\end{figure}
...
\begin{figure}[H]
    \centering
    \missingimage{classes.jpeg}{Diagramme de classes}
...
\begin{figure}[H]
    \centering
    \missingimage{BCaaS.png}{Pile Protocolaire}
...
% ============================================================
%  SECTION 6 — IMPLÉMENTATION FRONTEND ET SIMULATION API
% ============================================================
\newpage
...
\begin{figure}[H]
  \centering
  \missingimage{struct.jpeg}{Structure du projet \textit{Painting Business Core}}
